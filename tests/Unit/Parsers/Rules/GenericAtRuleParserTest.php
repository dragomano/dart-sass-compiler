...
describe('GenericAtRuleParser', function () {
    ...
    it('parses at-rule with numeric values', function () {
        $parser = ($this->createParser)('@counter-style thumbs { system: cyclic; }');

        $result = $parser->parse();

        expect($result)->toBeInstanceOf(AtRuleNode::class)
            ->and($result->name)->toBe('@counter-style')
            ->and($result->value)->toBe('thumbs');
    });
});
