...
describe('ColorFormat', function () {
    ...
    describe('formatFromFunction()', function () {
        it('formats HSL color from components', function () {
            $result = ColorFormat::formatFromFunction('hsl', [120, 50, 75], 0.8);

            expect($result)->toBe('hsla(120, 50%, 75%, 0.8)');
        });

        it('formats LCH color from components', function () {
            $result = ColorFormat::formatFromFunction('lch', [60, 40, 30]);

            expect($result)->toBe('lch(60% 40 30)');
        });

        it('formats LAB color from components', function () {
            $result = ColorFormat::formatFromFunction('lab', [50, 20, -10], 0.9);

            expect($result)->toBe('lab(50% 20 -10 / 0.9)');
        });

        it('formats HWB color from components', function () {
            $result = ColorFormat::formatFromFunction('hwb', [240, 10, 20]);

            expect($result)->toBe('hwb(240 10% 20%)');
        });

        it('formats OKLCH color from components', function () {
            $result = ColorFormat::formatFromFunction('oklch', [0.7, 0.15, 180], 0.5);

            expect($result)->toBe('oklch(70% 0.15 180 / 0.5)');
        });

        it('converts string components to float', function () {
            $result = ColorFormat::formatFromFunction('hsl', ['120', '50', '75']);

            expect($result)->toBe('hsl(120, 50%, 75%)');
        });

        it('throws exception for unsupported function', function () {
            expect(fn() => ColorFormat::formatFromFunction('invalid', [1, 2, 3]))
                ->toThrow(InvalidArgumentException::class, 'Unsupported color function: invalid');
        });

        it('throws exception for unsupported format for components', function () {
            expect(fn() => ColorFormat::formatFromFunction('hex', [255, 0, 0]))
                ->toThrow(InvalidArgumentException::class, 'Unsupported format for components: hex');
        });
    });
